# geno-tools `tt` shell layer.
#
# Source this from your shell profile (geno_tools/scripts/bootstrap.sh adds the
...
#
#   1. `cd` into a session/workspace/worktree — `geno-tools tt` writes the cd
...
tt() {
    ...
    TT_EXEC_FILE="$TT_EXEC_FILE" geno-tools tt "$@"
    local tt_exit=$?
...
" 2>/dev/null)"
    ...
    _tt_iterm2_apply_theme_live() {
        "$_TT_PYTHON" -m geno_tools.tt.themes --apply-live "$1" 2>/dev/null
    }
