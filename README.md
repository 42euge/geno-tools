...
Some slash commands still live in this repo's `commands/` — `gt-research`, `gt-lab-notes`, `gt-start-task`, `gt-rewrite-commit`, `gt-config-colab`, `gt-upload-colab`, `gt-upload-kaggle`. These will migrate into `geno-research`, `geno-dev`, and `geno-kaggle` as those repos come online.
