...
- `your-<name>` — hypothetical private skillset under an `your` namespace
...
1. **Curated registry** — submit a PR adding `"<repo-name>": "<git-url>"` to `genotools/registry.py`. After that, `geno-tools skills install <repo-name>` works for everyone.
...
4. **Mix public and private freely**. A developer can run `geno-tools skills install geno-<name>` (public) alongside `geno-tools skills install git@github.acme.com:platform/acme-<name>.git` (private) on the same machine. They share `~/.geno-tools/`, the same venv strategy, and the same slash-command surface in Claude Code / Codex / Antigravity CLI.
