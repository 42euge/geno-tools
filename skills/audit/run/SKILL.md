...
Skills may be organized into category directories, arbitrarily deep:
`skills/<category>/<name>/SKILL.md` (e.g. `skills/meta/harness/fork/SKILL.md`).
Discovery uses `npx skills add --full-depth` (geno-tools passes this).

**Required:**
- **Category-XOR-leaf**: every directory under `skills/` is EITHER a category
  (has child skill dirs, NO `SKILL.md` of its own) OR a leaf (has `SKILL.md`, no
  skill children). A directory that is both **fails** — its own `SKILL.md` shadows
  every nested skill, silently dropping them. Fix: move the parent's content into a
  child leaf (`vaults/SKILL.md` → `vaults/status/SKILL.md`).
- **Name mirrors path**: a leaf's frontmatter `name:` is the fully-qualified
  dotted-by-hyphen path from the skillset root — `skills/meta/harness/fork/` →
  `name: <skillset>-meta-harness-fork`. The `name` must be globally unique (leaf dir
  names like `install`/`run` legitimately repeat across categories; the FQ name does not).

**Recommended:**
- Sub-skillset/category segments are pluralized nouns or domains (not verbs)
- The leaf segment is an action verb
- The umbrella skill's `description` lists the categories and their commands
