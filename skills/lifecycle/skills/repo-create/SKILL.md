...
Construct `geno-{slug}`. The slug must be lowercase, hyphen-separated if multi-word, a noun, and not already taken — check with `geno-tools ls --available 2>/dev/null` and `ls ~/.geno-tools/`.
